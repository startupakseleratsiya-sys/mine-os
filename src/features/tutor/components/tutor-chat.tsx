...
function TypingDots() {
    return (<div className="flex items-center gap-1 px-1 py-1">
      {[0, 1, 2].map((i) => (<span key={i} className="size-1.5 rounded-full bg-[#65736d] animate-bounce" style={{ animationDelay: `${i * 0.15}s`, animationDuration: "0.9s" }}/>))}
    </div>);
}
/* Web Speech API brauzer tiplari (lib.dom'da yo'q) — faqat ishlatiladigan qismi. */
type SpeechResultLike = ArrayLike<{
    transcript: string;
}> & {
    isFinal: boolean;
};
...
export function TutorChat({ embedded = false, context, }: {
    ...
    const sendMessage = useCallback(async (text: string) => {
        ...
        try {
            ...
            while (true) {
                ...
                received += chunk;
                setMessages((current) => current.map((msg) => (msg.id === assistantId ? { ...msg, content: msg.content + chunk } : msg)));
            }
            if (!received.trim()) {
    ...
    return (<div className={`flex flex-col bg-[#f3f1eb] text-[#13251f] ${embedded ? "h-full" : "h-dvh"}`}>
          ...
          <div className="mx-auto flex h-16 max-w-[900px] items-center justify-between px-4 sm:px-6">
            <div className="flex items-center gap-3">
              ...
              <Link href="/" className="flex items-center gap-2">
                ...
                <span className="font-semibold tracking-tight"><>{t("finora")}</></span>
              </Link>
            </div>
        ...
        <div className={`mx-auto max-w-[900px] px-4 sm:px-6 ${embedded ? "py-4" : "py-6"}`}>
          ...
          {messages.length === 0 ? (<div className={`flex flex-col items-center justify-center text-center ${embedded ? "py-8" : "py-10"}`}>
              ...
              {messages.map((message) => (<div key={message.id} className={`flex gap-3 ${message.role === "user" ? "justify-end" : "justify-start"}`}>
                  ...
                  <div className={`max-w-[85%] rounded-2xl px-4 py-3 sm:px-5 sm:py-4 ${message.role === "user"
                    ? "rounded-tr-sm bg-[#dce7dd] text-[#13251f] whitespace-pre-wrap text-sm leading-6"
                    : "rounded-tl-sm border border-[#13251f]/8 bg-white"}`}>
                    {message.role === "user" ? (message.content) : message.content ? (<Markdown text={message.content}/>) : (<TypingDots />)}
                  </div>
                  {message.role === "user" && (<span className="mt-1 grid size-8 shrink-0 place-items-center rounded-xl border border-[#13251f]/10 bg-white">
      ...
      <div className="shrink-0">
        <div className={`mx-auto max-w-[900px] px-4 pb-4 pt-2 sm:px-6 ${embedded ? "" : "sm:pb-5 sm:pt-3"}`}>
          {error && (<div role="alert" className="mb-3 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
              {t(error)}
            </div>)}
