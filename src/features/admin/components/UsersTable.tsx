...
export async function UsersTable({ limit }: {
    ...
    if (error) {
        return (<div className="rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700"><>{t("Foydalanuvchilarni yuklab bo'lmadi:")}</>{error.message}
      </div>);
    }
    ...
    return (<div className="overflow-x-auto rounded-xl border border-[#13251f]/10 bg-white">
      <table className="w-full text-left text-sm">
        <thead className="border-b border-[#13251f]/10 bg-[#f8f7f2]">
          <tr>
            <th className="px-6 py-4 font-medium text-[#65736d]"><>{t("Foydalanuvchi")}</></th>
            <th className="px-6 py-4 font-medium text-[#65736d]"><>{t("Rol")}</></th>
            <th className="px-6 py-4 font-medium text-[#65736d]"><>{t("Sana")}</></th>
          </tr>
        </thead>
