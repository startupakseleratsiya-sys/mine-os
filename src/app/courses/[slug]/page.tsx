        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6">
          ...
          <div className="flex flex-wrap items-center gap-3 mb-4">
            ...
            <span className="flex items-center gap-1.5 text-sm text-[#6B7A74] font-medium">
              <BookOpen className="size-4" /> {course.chapters.length} bob
            </span>
            <span className="flex items-center gap-1.5 text-sm text-[#6B7A74] font-medium">
              <Clock3 className="size-4" /> {formatMinutes(courseMinutes(course))}
            </span>
          </div>
          <h1 className="text-3xl sm:text-5xl font-extrabold tracking-tight text-[#0f2017] mb-4">{course.title}</h1>
          <p className="text-[#65736d] text-lg leading-relaxed max-w-2xl mb-8">{course.description}</p>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-4">
            ...
            >
              {cp.completed === 0 ? "Kursni boshlash" : cp.nextChapter ? "Davom ettirish" : "Qayta ko'rish"}
              <ArrowRight className="size-4" />
            </Link>
            {cp.completed > 0 && (
              <div className="flex items-center gap-3 text-sm text-[#6B7A74]">
        ...
        <div className="md:col-span-2 space-y-12">
          <section>
            <div className="flex items-center gap-2 mb-6">
              ...
              <h2 className="text-2xl font-bold text-[#0f2017]">O&apos;quv dasturi</h2>
            </div>
            <div className="bg-white rounded-3xl border border-[#E2E4DF] overflow-hidden">
              {course.chapters.map((ch, index) => {
                ...
                return (
                  <Link
                    key={ch.id}
                    href={`/study/${slug}/${ch.id}`}
                    className={`flex items-center justify-between gap-3 p-5 hover:bg-[#F5F4EE] transition-colors ${
                      index !== course.chapters.length - 1 ? "border-b border-[#E2E4DF]" : ""
                    }`}
                  >
                    <div className="flex items-start gap-4 min-w-0">
                      <span
                        className={`grid size-8 shrink-0 place-items-center rounded-full text-sm font-bold ${
                          done ? "bg-emerald-100 text-emerald-700" : "bg-[#dce7dd] text-[#285744]"
                        }`}
                      >
                        {done ? <CheckCircle2 className="size-4" /> : index + 1}
                      </span>
                      <div className="min-w-0">
                        <h3 className="font-semibold text-[#0f2017]">{ch.title}</h3>
                        <p className="text-xs text-[#6B7A74] mt-1">{ch.minutes} daqiqa{done ? " · tugatilgan" : ""}</p>
                      </div>
                    </div>
                    <span className="text-sm font-bold text-[#163e32] shrink-0">{done ? "Qayta o'qish" : "O'qish →"}</span>
                  </Link>
                );
              })}
            </div>
          ...
          <div className="bg-white rounded-3xl border border-[#E2E4DF] p-6 shadow-sm">
            ...
            <ul className="space-y-3">
              {course.outcomes.map((outcome) => (
                <li key={outcome} className="flex items-start gap-2.5 text-sm text-[#65736d] leading-relaxed">
