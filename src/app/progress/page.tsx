  ...
  return (
    ...
    >
      <ProgressView
        ...
        minutesLearned={totalMinutesLearned(rows)}
        streak={computeStreak(rows)}
        weekly={weeklyActivity(rows).map(({ day, lessons, minutes, isToday }) => ({ day, lessons, minutes, isToday }))}
        recent={recentLessons(rows).map((r) => ({
          title: r.chapter.title,
          course: r.course.shortTitle,
          when: relativeDay(r.completedAt),
          href: `/study/${r.course.slug}/${r.chapter.id}`,
        }))}
        achievements={achievements(rows, chatCount)}
        active={{
